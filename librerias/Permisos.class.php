...
class Permisos {
  ...
  function politicas($permiso){
    ...
    $sql="SELECT  `aplicacion_politicas`.`rol`,`aplicacion_roles`.`nombre`
      FROM `aplicacion_roles`
      INNER JOIN `aplicacion_politicas` ON `aplicacion_politicas`.`rol`=`aplicacion_roles`.`rol`
      WHERE `permiso`='" . $permiso . "'
      GROUP BY `aplicacion_politicas`.`rol`,`aplicacion_roles`.`nombre`;";
    $consulta = $db->sql_query($sql);
